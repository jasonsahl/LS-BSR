...
#### Recent changes:
1. USEARCH has been removed as a clustering option. In its place, mmseqs2 is now available
for clustering, as well as the linear clustering method.
2. I recently repeated some analyses to compare LS-BSR with Roary and the results are [here](https://github.com/jasonsahl/LS-BSR/wiki/Some-thoughts-on-comparing-Roary-to-LS-BSR)
3. LS-BSR now supports the use of Diamond and BLASTP for protein-protein comparisons.
...
4. Default parameters for Prodigal have been changed to prevent reading through large
stretches of Ns.
5. LS-BSR can now compare intergenic regions if the “-y” flag is set to “T” and if a nucleotide
aligner is chosen.
6. All functionality should now be compatible with Python 2.7 through 3.5. Seems to work with
3.6, although some additional testing is required.
7. Added “-z” toggle that can turn off duplicate searching, which can speed up very large
analyses (v1.0.4)
8. Added blastn-short as an alignment option (v1.0.5)
...
#### Dependencies:  
1. Python >2.7 and <=3.5; later versions of Python are compatible, but the tests will fail  
2. mmseqs2 [optional] (current tested version is 10.6d92c) - At least one clustering method
must be chosen if a set of genes is not supplied. The easiest installation method is through
conda [https://anaconda.org/bioconda/mmseqs2]. If you invoke with "-c mmseqs", it runs the
"easy-cluster" method; if the "-c mmseqs-lin" is invoked, it runs the "easy-linclust" method.  
...
supplied. Can be freely obtained at: [https://anaconda.org/bioconda/vsearch]. Currently, VSEARCH
...
from: [https://anaconda.org/bioconda/cd-hit]  
...
BLAT. Blast+ can be obtained from: [https://anaconda.org/bioconda/blast]. Weird and incorrect bit scores have been observed with 2.2.31. If BLASTP is used, the coding regions are clustered, translated, and aligned against proteins predicted by Prodigal. This is a
...
diamond protein comparisons. Can be obtained from: [https://anaconda.org/bioconda/diamond]  
...
“mmseqs”, "mmseqs-lin", “vsearch”, or “cd-hit”. These must be in your path as “mmseqs”, “vsearch”, “cd-hit-est”, or “cd-hit” to use.  
...
3. Test the de novo gene prediction method with mmseqs and TBLASTN for alignment:  
```python /Users/jsahl/LS-BSR/ls_bsr.py -d genomes –c mmseqs –x test```  
...
-you could then cluster and use the resulting centroids in your LS-BSR
...
pan-genome profiles  
-What do you need for the script to run?  
• Numpy in your PYTHONPATH; can be installed with
```conda install -c conda-forge numpy```  
• Pandas in your PYTHONPATH  
```conda install -c conda-forge pandas```  
• Scipy in your PYTHONPATH  
```conda install -c conda-forge scipy```  
• Sci-kit bio in your PYTHONPATH  
